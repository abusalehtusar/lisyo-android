package dev.abu.lisyo.ui.theme

import androidx.compose.ui.graphics.Color

val OceanDark = Color(0xFF000F14) // Darker background (almost black)
val OceanPrimary = Color(0xFF00B4D8) // Light Blue
val OceanSecondary = Color(0xFF90E0EF) // Lighter Blue
val OceanTertiary = Color(0xFFCAF0F8) // Foam
val OceanSurface = Color(0xFF001E2B) // Slightly lighter for surface
val OnOceanDark = Color(0xFFFFFFFF)

// New Button Palette
val DeepBlue = Color(0xFF005F73) // Darker selected state
val SoftWhite = Color(0xFFF8FAFC) // Whiter unselected state
val BorderLight = Color(0xFFE2E8F0) // Light border

// Updated Palette
val PurpleSelected = Color(0xFF403266)
val PurpleBrighter = Color(0xFFCBC0E6)
