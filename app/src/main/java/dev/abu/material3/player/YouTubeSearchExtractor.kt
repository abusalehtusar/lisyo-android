...
object YouTubeSearchExtractor {
    ...
    // Using WEB_REMIX for YT Music results
    ...
    suspend fun search(query: String): List<Song> {
        try {
            ...
            val json = JSONObject().apply {
                put("query", query)
                put("params", "EgWKAQIIAWoKEAkQBRAKEAMQBA==") // Filter for Songs in YT Music
            ...
            return parseMusicSearchResponse(searchResponse)
        } catch (e: Exception) {
    ...
    private fun parseMusicSearchResponse(response: JSONObject): List<Song> {
        ...
        try {
            ...
            for (i in 0 until contents.length()) {
                ...
                for (j in 0 until items.length()) {
                    ...
                    val song = parseMusicRenderer(renderer)
                    if (song != null) {
                        songs.add(song)
                    }
                }
            ...
            Logger.logError(TAG, "Error parsing music search response", e)
        }
        return songs
    }

    private fun parseMusicRenderer(renderer: JSONObject): Song? {
        try {
            ...
            if (secondColumnRuns != null) {
                // Look for duration run
                for (i in 0 until secondColumnRuns.length()) {
                    val text = secondColumnRuns.optJSONObject(i).optString("text")
                    if (text.contains(":")) {
